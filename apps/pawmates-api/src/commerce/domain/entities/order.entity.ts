...
export class Order {
  ...
  // A ULID (Booking.id's own type), not a real UUID — same reason every
  // other aggregate root's id in this repo is `text`, not `uuid`.
  @Column({ name: 'delivery_booking_id', type: 'text', nullable: true })
