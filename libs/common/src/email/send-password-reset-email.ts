...
export async function sendPasswordResetEmail(
  ...
  if (!apiKey) {
    console.warn(
      `[email] RESEND_API_KEY no configurado — enlace de restablecimiento para ${to}: ${resetLink}`,
    );
    return;
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Resend respondió ${res.status} al enviar el correo de restablecimiento: ${body}`,
    );
  }
}
